...
- **Vault** 
  Knows everything about the overall Obsidian folder structure → lists all .md files, lists all images, handles path normalization
- **Note** 
  Represents the single markdown file we’re working on → extracts all image references from its content (using regex on `![[...]]` and `[[...]]` links)
- **ImageMover** 
  The “business logic” coordinator → finds images that are **unique** to our note → creates the destination folder (user prompt or default name) → safely moves the note + its exclusive images